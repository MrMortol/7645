...
class Freekassa
{
    ...
    public function getResult()
    {
        ...
        {
            return $this->getResponseError('Ошибка! Вы не заполнили необходимые поля.');
        }

        if ($request['SIGN'] != md5(Config::MERCHANT_ID.':'.$request['AMOUNT'].':'.Config::SECRET_KEY_2.':'.$request['MERCHANT_ORDER_ID']))        
        {
            return $this->getResponseError('Системная ошибка! Вернитесь на страницу оплаты и попробуйте еще раз.');
        }
        ...
        if ($freekassaModel->getPaymentByFreekassaId($request['intid']))
        {
            ...
            return $this->getResponseSuccess('Платеж уже сущестует');
        }
    ...
    private function GoTofreekassa($request)
    {
        ...
        $o = 'Пополнение баланса аккаунта ' . $us_account . '';
